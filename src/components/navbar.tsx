  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        {/* Desktop Nav */}
        <nav className="hidden xl:flex items-center gap-1 bg-zinc-900/40 p-1.5 rounded-full border border-white/5 backdrop-blur-md">
          {navLinks.map((link) => {
              ...
              >
                {isActive && (
                  <motion.span
                    ...
                    className="absolute inset-0 bg-gradient-to-r from-emerald-600/30 to-emerald-900/30 border border-emerald-500/40 rounded-full"
        ...
        <div className="xl:hidden flex items-center">
          <button
            ...
            className="p-2 text-zinc-400 hover:text-white rounded-lg bg-zinc-900/50 border border-white/10 focus:outline-none"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="xl:hidden bg-[#0a0a0a]/95 backdrop-blur-2xl border-b border-emerald-500/20 shadow-2xl overflow-hidden"
          >
            <div className="px-6 py-6 grid grid-cols-2 gap-3 max-w-lg mx-auto">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  onClick={(e) => handleNavClick(e, link.href)}
                  className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all text-center border ${
                    activeSection === link.href.substring(1)
                      ? 'bg-emerald-950/40 border-emerald-500/40 text-emerald-300 font-semibold'
                      : 'bg-zinc-900/40 border-white/5 text-zinc-300 hover:bg-zinc-800/50'
                  }`}
                >
                  {link.name}
                </a>
              ...
              <div className="col-span-2 pt-2">
                <a
                  href="#contact"
                  onClick={(e) => handleNavClick(e, '#contact')}
                  className="w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 shadow-lg shadow-emerald-900/40"
                >
